...
- Filenames are preserved, including punctuation such as `?`, `:`, `#`, `@`, quotes, and brackets, Unicode, and trailing dots/spaces. The destination filesystem must support these names (Windows filesystems impose additional restrictions); SilverBullet links may need escaping or may not address every such name. The importer does not rename files or rewrite links.
