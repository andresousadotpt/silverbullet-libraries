...
- Filenames SilverBullet can write are preserved, including `?`, `:`, quotes, Unicode, and trailing dots/spaces. SilverBullet itself rejects names that its reference syntax interprets specially—such as `wsl.conf.md`, or names containing `#`, `@`, `|`, `<`, `>`, or `$`. The importer gives only those files a readable compatible alias (for example, `wsl.conf.md` becomes `wsl_conf.md`) and records every mapping in the report. Existing links are not rewritten.
- Unsafe paths reject the archive: absolute/parent paths, Windows drive paths, backslashes, control characters, and empty or `.` path segments. Duplicate paths, Unicode/case-equivalent paths, and file/folder collisions inside the ZIP reject the plan instead of choosing a winner.
- Limits: 1 GiB compressed ZIP, 2 GiB declared extracted content, 512 MiB per file, and 10,000 entries including directories. Actual extraction sizes and CRC checks are verified. Password-protected entries are unsupported. Files are extracted one at a time into a single output buffer, but the native picker still reads the entire selected ZIP into memory before its size can be checked. Large vaults need sufficient browser memory; these limits are not a guarantee that every device can import a maximum-size archive.
- A failure for one file is reported while the importer continues with later files; a read-only space stops further writes. There is no automatic rollback. Rerun to skip completed files. Imports use SilverBullet's file API; a successful API response is not independent verification of server persistence.
