...
async function fixture() {
  ...
  await writer.add('Vault/Notes/deep/Café.md', new Uint8ArrayReader(new TextEncoder().encode('# Café\r\n[[Other]]')));
...
for (const scenario of ['import', 'read-only', 'cancel', 'failure', 'existing'] as const) {
  test(`compiled Obsidian import command: ${scenario}`, async ({ page }) => {
    ...
    if (scenario === 'read-only' || scenario === 'cancel') {
      ...
      expect(result.writes).toEqual([
        { path: 'Imported/Notes/deep/Café.md', bytes: Array.from(new TextEncoder().encode('# Café\r\n[[Other]]')) },
