...
The editor starts in preview mode. **Enable editing** creates an exact original backup in the same folder before allowing changes. Edits are sent through SilverBullet's normal autosave mechanism.
...
This is a lightweight data editor, not full Excel. Advanced formatting, charts, images, pivot tables, macros and other workbook features are not guaranteed to survive conversion or editing. Keep the original backups for complex workbooks. Supported formulas recalculate locally; unsupported formulas show an error and keep their formula text. CSV/TSV are UTF-8 text formats with one sheet and do not evaluate formulas. Read-only spaces and files remain read-only.
